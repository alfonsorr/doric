...
class StringColumnsSpec extends DoricTestElements {
  ...
  describe("split doric function with limit") {
    import spark.implicits._
  ...
  describe("schemaOfCsv doric function") {
    ...
    val df = List("column not read").toDF("col1")
    // Spark 4.0 format
    val expected = List(Some("STRUCT<_c0: STRING, _c1: STRING>"))
  ...
  describe("schemaOfJson doric function with options") {
    ...
    it("should work as spark schema_of_json function with options") {
      // Spark 4.0 format
      val expected = List(Some("ARRAY<STRUCT<col: BIGINT>>"))
